...
class GrinderDataAccess {
    ...
    func GetGrinderList(completion: @escaping (Result<[GrinderDto], Error>) -> Void) {
        // Build API request
        guard let url = URL(string: "http://localhost:5100/grinder") else {
            completion(.failure(URLError(.badURL)))
            return
        }
        ...
        URLSession.shared.dataTask(with: url) { data, response, error in
            ...
            let decoder = JSONDecoder()
            do {
                let grinderDtoList = try decoder.decode([GrinderDto].self, from: data)
                completion(.success(grinderDtoList))
            } catch {
